...
결국 이는 화면(View)과 서비스(Service)의 책임이 분리되지 않아서 발생하는 일로, 이를 분리하기 위해 **템플릿 엔진** 을 사용한다. 최근에는 스프링 진영에서 대표적인 템플릿 엔진으로 타임리프를 사용하지만, 당시에는 JSP 를 많이 사용하였으므로 이번 프로젝트에서도 JSP 를 이용해보자. Spring Boot 프로젝트에서 JSP 를 사용하기 위해서는 다음 의존성들을 추가해야 한다.
